...
export default function Page() {
  ...
  const setupFirebaseListeners = () => {
    try {
      // Reservaties sync
      const unsubReservations = syncData.onReservationsChange((data) => {
        console.log('🔥 Firebase: Reservaties geüpdatet', data);
        setReservations(data);
        localStorage.setItem(RESERV_KEY, JSON.stringify(data));
        setFirebaseConnected(true);
      });

      // Match types sync
      const unsubMatchTypes = syncData.onMatchTypesChange((data) => {
        console.log('🔥 Firebase: Match types geüpdatet', data);
        setMatchTypes(data);
        localStorage.setItem(MATCHTYPE_KEY, JSON.stringify(data));
      });

      // Beschikbaarheid sync
      const unsubAvailability = syncData.onAvailabilityChange((data) => {
        console.log('🔥 Firebase: Beschikbaarheid geüpdatet', data);
        setAvailability(data);
        localStorage.setItem(AVAIL_KEY, JSON.stringify(data));
      });

      // Geselecteerde datum sync
      const unsubSelectedDate = syncData.onSelectedDateChange((date) => {
        console.log('🔥 Firebase: Datum geüpdatet', date);
        if (!isUpdatingDate) {
  ...
  /* --- Persist (lokaal) --- */
  useEffect(() => {
    if (syncEnabled && firebaseConnected) {
  ...
  useEffect(() => {
    if (syncEnabled && firebaseConnected) {
      syncData.setMatchTypes(matchTypes);
    }
    localStorage.setItem(MATCHTYPE_KEY, JSON.stringify(matchTypes));
  }, [matchTypes]);
  
  useEffect(() => {
    if (syncEnabled && firebaseConnected && !isUpdatingDate) {
      setIsUpdatingDate(true);
      syncData.setSelectedDate(selectedDate);
      setTimeout(() => setIsUpdatingDate(false), 100);
    }
  ...
  useEffect(() => {
    if (syncEnabled && firebaseConnected) {
      syncData.setAvailability(availability);
    }
  ...
  const planAllBalanced = async () => {
    ...
    await Promise.all(
      result.map((r) =>
        setDoc(
          doc(db, 'reservations', `${r.date}_${r.timeSlot}_${r.court}`),
          r,
          {
            merge: true,
          }
    ...
    <div className="min-h-screen bg-gray-100 p-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-3 mb-6">
          <div>
            ...
            {firebaseConnected && (
              <p className="text-xs text-green-600 mt-1">
                ✅ Real-time sync actief - wijzigingen worden direct gedeeld
              </p>
            )}
            {!firebaseConnected && syncEnabled && (
          ...
          <div className="flex items-center flex-wrap gap-2">
            <button
              ...
              title={syncEnabled ? 'Live sync uitschakelen' : 'Live sync inschakelen'}
            >
              {syncEnabled ? '🔄 Live' : '📱 Lokaal'}
            </button>
            {isAdmin && (
              <>
                <button
        ...
        <div className="mt-6 text-center text-sm text-gray-500">
          <p>💡 Tip: Je data worden {firebaseConnected ? 'real-time gesynchroniseerd én lokaal opgeslagen' : 'lokaal opgeslagen in je browser'}.</p>
          <p>📱 Deze pagina is geoptimaliseerd voor smartphone.</p>
          {firebaseConnected && (
            <p className="text-green-600 text-xs mt-1">
              🔥 Firebase real-time database actief
            </p>
          )}
        </div>
        <br />
